...
class Latin_Noun extends Noun_Common_IE {
    ...
    // 名詞情報を取得
    protected function set_data($noun){
    	// 名詞情報を取得
...
// 古ラテン語名詞クラス
class Old_Latin_Noun extends Latin_Noun {
	protected $case_suffix_list =  [
		[
			"noun_type" => "1",
			"noun_type_name" => "第一活用",				
    ...
    コンストラクタ
    ======================================*/
    public function __construct_old_lat1($noun) {
    	// 親クラス初期化
		parent::__construct($noun);
		// 語幹を修正
		$this->convert_to_old_latin($this->first_stem);	// 第1語幹
		$this->convert_to_old_latin($this->third_stem);	// 第3語幹
    }
    
    /*=====================================
    コンストラクタ
    ======================================*/
    public function __construct_old_lat3($compound, $last_word, $translation) {
    	// 親クラス初期化
		parent::__construct($compound, $last_word, $translation);
		// 語幹を修正
		$this->convert_to_old_latin($this->first_stem);	// 第1語幹
		$this->convert_to_old_latin($this->third_stem);	// 第3語幹
    }
    ...
    public function __construct() {
        ...
        $i = func_num_args();
        //引数に応じて別々のコンストラクタを似非的に呼び出す
        if (method_exists($this,$f='__construct_old_lat'.$i)) {
            call_user_func_array(array($this,$f),$a);
        }
    }

	// 古ラテン語に変換
	private function convert_to_old_latin($word){
		// 変換後の文字列を宣言
		$word = "";

		// 変換開始(母音)
		$word = preg_replace("/ae/u", "ai", $word);
		$word = preg_replace("/ī/u", "ei", $word);
		$word = preg_replace("/ū/u", "ou", $word);
		$word = preg_replace("/oe/u", "oi", $word);

		// 変換開始(子音)
		// r変化の場合は
		if($this->noun_type == "3r"){
			$word = preg_replace("/or$/u", "ōs", $word);
		}
		
		// r変化(s→r)の場合は
		if($this->noun_type == "3Rhot"){
			$word = preg_replace("/r$/u", "s", $word);
		}
