...
<html lang="ja">
  ...
  <body>
    ...
    <div class="container item table-striped">
      <h1>ポーランド語辞書（形容詞曲用検索）</h1>
      <div class="mt-4 mb-4" id="form-search">
        <section class="row">
          <div class="col-md-12 mb-0 textBox1">
            <input type="text" name="word" id="word" class="form-control" placeholder="形容詞の辞書形を入れてください">
            <?php echo Polish_Common::input_special_button(); ?>
          </div>
        </section>
        <?php echo Polish_Common::number_selection_button(false); ?> 
        <?php echo Polish_Common::case_selection_button(false); ?>
        <?php echo Polish_Common::adjective_gender_selection_button(false); ?>
        <?php echo Polish_Common::adjective_grade_selection_button(false); ?>
        <?php echo Polish_Common::declension_button_and_text(); ?>
      </div>
    </div>
  <footer class="">
  </footer>
    ...
    <script>
      ...
      function setEvents(){
          ...
          // 語形変化を検索
          Word_Change_Search.search_adjective_declension("polish");
	      });
